...
import { createStudyMaterial } from '@/lib/study-materials-api';
...
export default function NewStudyMaterialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'فشل إضافة الأداة الدراسية');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <ProtectedPage permission={{ page: 'dashboard.study-materials', action: 'create' }}>
  ...
  return (
    <ProtectedPage permission={{ page: 'dashboard.study-materials', action: 'create' }}>
      <div className="space-y-6">
        <PageHeader
          title="إضافة أداة دراسية جديدة"
          description="إضافة أداة أو مستلزم دراسي جديد"
          breadcrumbs={[
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
              <div className="flex flex-col sm:flex-row justify-end gap-3">
                ...
                <Button type="submit" variant="primary" isLoading={submitting} className="w-full sm:w-auto" leftIcon={<PlusIcon className="w-4 h-4" />}>
                  إضافة الأداة
                </Button>
              </div>
            </div>
