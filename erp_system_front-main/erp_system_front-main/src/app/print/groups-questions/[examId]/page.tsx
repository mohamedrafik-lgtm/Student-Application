...
export default function GroupsQuestionsPrintPage({ params }: { params: Promise<{ examId: string }> }) {
            ...
            return (
              <div key={committee.committeeNumber}>
                ...
                <div className="min-h-screen flex flex-col items-center justify-center page-break">
                  <div className="text-center">
                    <div className="mb-4">
                      <div className="inline-block bg-slate-700 text-white px-8 py-4 rounded-xl shadow-sm">
                        <h1 className="text-4xl font-black mb-1">{group.groupName}</h1>
                        <h2 className="text-3xl font-bold">اللجنة {committee.committeeNumber}</h2>
                        <p className="text-xl mt-2">{committee.sheetsCount} طالب</p>
                      </div>
                    </div>
                    
                    <div className="mt-8 bg-white border-4 border-teal-200 rounded-xl p-6 inline-block">
                      <p className="text-xl font-bold mb-3 text-slate-800">النماذج المستخدمة:</p>
                      <div className="space-y-2">
                        {Array.from(committeeModels.entries()).map(([modelId, info]: [any, any]) => (
                          <div key={modelId} className="flex items-center gap-3 text-lg">
                            <span className="font-black text-teal-600 text-2xl">{info.model.modelCode}</span>
                ...
                {/* طباعة كل نموذج مرة واحدة لكل لجنة */}
                {Array.from(committeeModels.entries()).map(([modelId, info]: [any, any], modelIndex: number) => {
                  const modelData = modelsData[modelId];
                  if (!modelData) return null;

                  const isLastModel = modelIndex === committeeModels.size - 1;
                  const isLastCommittee = committeeIndex === group.committees.length - 1;
                  const isLastGroup = groupIndex === data.groups.length - 1;

                  return (
                  <div key={modelId} className={`p-8 bg-white max-w-4xl mx-auto ${!isLastModel || !isLastCommittee || !isLastGroup ? 'page-break' : ''}`}>
                    ...
                    <div className="text-center mb-8 border-b-4 border-black pb-4">
                      ...
                      <div className="flex justify-around items-center mt-4 pt-4 border-t-2 border-slate-300">
                        <p>نموذج: <span className="font-bold text-xl">{info.model.modelCode}</span></p>
                        <p>المجموعة: <span className="font-bold text-xl text-teal-600">{group.groupName}</span></p>
                    ...
                    <div className="mt-8 pt-4 border-t-2 border-black text-center">
                      ...
                      <p className="text-xs text-slate-600 mt-2">
                        {group.groupName} - اللجنة {committee.committeeNumber} - النموذج {info.model.modelCode} - {info.count} طالب
                      </p>
                    </div>
                  </div>
                  );
