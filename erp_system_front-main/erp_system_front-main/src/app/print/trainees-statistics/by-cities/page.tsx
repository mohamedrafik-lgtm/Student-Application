...
// تحويل كود المدينة إلى اسم عربي
const getCityLabel = (governorateValue: string, cityValue: string): string => {
  const governorate = egyptGovernoratesAndCities.find(g => g.value === governorateValue);
  const city = governorate?.cities.find(c => c.value === cityValue);
  return city?.label || cityValue || 'غير محدد';
};

export default function PrintTraineesStatisticsByCitiesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // تجميع الإحصائيات حسب المدينة
        const statsMap = new Map<string, any>();
        
        trainees.forEach((trainee: any) => {
          const cityValue = trainee.city || trainee.address?.city || 'غير محدد';
          const governorateValue = trainee.governorate || trainee.address?.governorate || '';
          const cityLabel = getCityLabel(governorateValue, cityValue);
          
          if (!statsMap.has(cityLabel)) {
            statsMap.set(cityLabel, { city: cityLabel, total: 0, males: 0, females: 0 });
          }
          
          const stats = statsMap.get(cityLabel);
          stats.total++;
  ...
  useEffect(() => {
    if (!isLoading && cityStats.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-white text-black" dir="rtl">
      <style jsx global>{`
        ...
        .data-table .city-name { text-align: right; font-weight: bold; }
      ...
      <div className="print-container">
        ...
        <div className="header">
          ...
          <h2 className="subtitle">إحصائية المتدربين حسب المدن</h2>
        ...
        <div className="stats-summary">
          ...
          <div className="stat-card" style={{borderColor: '#10b981'}}>
            <div className="stat-value" style={{color: '#10b981'}}>{totals.males}</div>
        ...
        <div style={{marginTop: '30px'}}>
          ...
          {cityStats.length === 0 ? (
            ...
            <table className="data-table">
              ...
              <tbody>
                {cityStats.map((stat: any, index: number) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className="city-name" style={{fontSize: '14px'}}>{stat.city}</td>
                      <td style={{fontWeight: 'bold', color: '#3b82f6', fontSize: '16px'}}>{stat.total}</td>
                      <td style={{fontWeight: 'bold', color: '#10b981', fontSize: '14px'}}>{stat.males}</td>
                ...
                <tr>
                  ...
                  <td style={{fontWeight: 'bold', color: '#10b981', fontSize: '16px'}}>{totals.males}</td>
