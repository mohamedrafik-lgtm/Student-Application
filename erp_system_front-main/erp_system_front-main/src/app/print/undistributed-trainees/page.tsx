...
export default function UndistributedTraineesPrint() {
  ...
  useEffect(() => {
    ...
    fetchData(programId, type);
  }, []);

  const fetchData = async (programId: string, type: string) => {
    try {
      const params = new URLSearchParams();
      params.append('limit', '10000'); // جلب الكل
      
      if (programId) params.append('programId', programId);
      if (type) params.append('type', type);

      const [traineesResponse, settingsData] = await Promise.all([
        fetchAPI(`/trainee-distribution/undistributed/trainees?${params.toString()}`),
        getSystemSettings(),
      ]);
  ...
  const typeLabel = filterType === 'THEORY' ? 'النظري' : filterType === 'PRACTICAL' ? 'العملي' : 'جميع الأنواع';
    ...
    <div className="min-h-screen bg-white text-black font-cairo" dir="rtl" style={{margin: 0, padding: 0}}>
      ...
      <div className="print-container">
        ...
        <div className="header">
          ...
          <h2 className="subtitle">المتدربون غير الموزعين - {typeLabel}</h2>
        ...
        <div className="section">
          <h3 className="section-title">قائمة المتدربين ({trainees.length} متدرب)</h3>
          
          {trainees.length === 0 ? (
            <div style={{textAlign: 'center', padding: '40px', color: '#666'}}>
              لا يوجد متدربين غير موزعين
            </div>
          ) : (
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th style={{width: '180px'}}>البرنامج</th>
                </tr>
              </thead>
              <tbody>
                {trainees.map((trainee, index) => (
                  <tr key={trainee.id}>
                    ...
                    <td style={{fontFamily: 'monospace', direction: 'ltr', textAlign: 'left'}}>{trainee.phone}</td>
                    <td style={{fontFamily: 'monospace', direction: 'ltr', textAlign: 'left'}}>{trainee.guardianPhone}</td>
        ...
        <div className="footer">
          <div style={{borderTop: '1px solid #ccc', paddingTop: '15px'}}>
            ...
            <p style={{margin: '5px 0'}}>نظام إدارة توزيع المتدربين</p>
